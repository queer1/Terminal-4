...
create_timeval_from_ms(unsigned timeout)
{
...
unsigned int
comm_get_cli_address(Socket *sock)
{
  return sock->cliaddr.sin_addr.s_addr;
}

unsigned int
comm_get_address(void)
{
  return 0;
}

static int
comm_bind(Socket *sock, int port)
{
...
static int
comm_listen(Socket *sock, int maxconn)
{
  int result = SOCKET_ERROR;
...
static int
get_available_bytes(Socket *sock)
{
  int rc;
  int timeout = 1000;
  int nfds = FD_SETSIZE;
...
int
send_bytes(Socket *sock, const unsigned char *data, int len)
{
  int sent = 0;
  while (sent < len)
    {
...
void
comm_send_bytes(Socket *sock, const unsigned char *data, int len)
{
  if (sock->bufsize)
    {
...
void
comm_sendto(Socket *sock, const char *hostname, int port)
{
...
int
comm_receive(Socket *sock, void *buf, size_t len)
{
...
int
comm_write(Socket *sock, const void *buf, size_t len)
{
  int result = SOCKET_ERROR;
  if (sock)
    {
      ...
      rc = select(nfds, NULL/*readfds*/, &writefds, NULL/*exceptfds*/, &teval);
      if (rc == SOCKET_ERROR)
        {
        }
      else if (rc == 0)
      ...
      else if (FD_ISSET( sock->sock, &writefds ))
        {
          ...
          if (rc == SOCKET_ERROR)
            ...
            { /* rc contains number of bytes sent, which may be less than len */
              result = rc;
            }
        }
    }
...
int
comm_read(Socket *sock, void *buf, size_t len, unsigned timeout)
{
  int result = SOCKET_ERROR;
  if (sock)
    {
...
void
comm_destroySocket(Socket *sock)
{
  if (sock)
    {
      close(sock->sock);
